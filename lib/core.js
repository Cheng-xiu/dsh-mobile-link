...
import { channelReady, CHANNEL_LABELS, configExists, loadConfig } from './store.js';
...
export function spawnDshWeb({ trustedHost, port, profile = null }) {
  ...
  if (profile && profile !== 'web') {
    // The installed launcher's `web` subcommand hardcodes profile "web", defines
    // no --profile option, and uses passThroughOptions — `dsh web --profile X`
    // would silently boot the wrong profile with stray args. The launcher only
    // honors --profile in the pre-subcommand (root) form below.
...
export async function runStart({ ctx, cordisConfig, options = {} }) {
  ...
  // Fail fast on missing push credentials before doing tunnel/verification work.
  // Skip the check only when the caller explicitly disabled the push step.
  if (options.push !== false && !channelReady(config)) {
    throw new Error('推送渠道凭证未配置，无法推送。请先运行: node bin/cli.js setup');
  }
  // config.json wins when present; a missing file yields DEFAULTS (webPort 3080)
  // which must not override an explicit cordis/plugin webPort.
  const hasFile = configExists();
  const port = options.port
    ?? (hasFile && Number.isInteger(config.webPort) ? config.webPort : null)
    ?? (cordisConfig && Number.isInteger(cordisConfig.webPort) ? cordisConfig.webPort : null)
    ?? 3080;
