...
import { assertAuthority, extractHostname, findOnPath, probePort, waitPort } from './util.js';
...
export async function runStart({ ctx, cordisConfig, options = {} }) {
  ...
  console.log('[1/3] 确保 cloudflared 隧道运行（转发 127.0.0.1:' + port + '）...');
  const tunnel = await ensureMobileUrl({ ctx, port });
  console.log('[2/3] 等待 DSH Web 就绪...');
  const ready = await waitPort(port, 90000);
  if (!ready) throw new Error('等待端口 ' + port + ' 超时，DSH Web 未就绪');
  console.log('[3/3] 通过 ' + CHANNEL_LABELS[config.channel] + ' 推送手机链接...');
