...
/**
 * Whether the persistent config file exists. loadConfig() returns DEFAULTS
 * when it is missing; callers must not let those defaults override explicit
 * cordis/plugin config values.
 */
...
function applyOptions(config, options) {
  ...
  // Commander maps `--no-auto` to options.auto === false (options['no-auto']
  // is never populated), so that is the correct off switch.
  if (options.auto === false) config.autoSend = false;
}
