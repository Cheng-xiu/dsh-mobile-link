...
export async function ensureTunnel(port, options = {}) {
  ...
  // The wait must be able to outlive the stale threshold, otherwise we would
  // give up before a dead holder's lock could ever be reclaimed.
  const deadline = Date.now() + Math.max(timeoutMs + 30000, LOCK_STALE_MS + 5000);
  let fd = null;
  while (Date.now() < deadline) {
    try {
      ...
      try {
        ...
        if (Date.now() - stat.mtimeMs > LOCK_STALE_MS) {
          // mtime alone is not proof of abandonment: verify the holder pid is
          // actually dead before reclaiming; otherwise keep waiting.
          let holder = null;
          try {
            holder = JSON.parse(fs.readFileSync(lockPath(), 'utf8').replace(/^\uFEFF/, ''));
          } catch { /* unreadable or invalid lock: fallback below */ }
          const holderPid = holder && Number.isInteger(holder.pid) ? holder.pid : null;
          if (holderPid === null) {
            // Unreadable lock or no pid recorded: fall back to the
            // mtime-based takeover as before.
            fs.unlinkSync(lockPath());
          } else if (!processAlive(holderPid)) {
            fs.unlinkSync(lockPath());
          }
        }
  ...
  try {
    ...
    try {
      // Only remove the lock file if it is still the exact one we created: a
      // stale takeover could recreate lockPath while we were finishing, and a
      // blind unlink would then delete the new holder's lock. Compare dev/ino
      // while we still hold our fd open, then unlink before closing.
      const ours = fs.fstatSync(fd);
      const current = fs.statSync(lockPath());
      if (current.dev === ours.dev && current.ino === ours.ino) fs.unlinkSync(lockPath());
...
export function stopTunnel(port = null) {
  ...
  // An explicitly requested port wins; only fall back to the state's port when
  // no port was specified (otherwise --stop --port 3099 could stop the 3080 tunnel).
  const targetPort = Number.isInteger(port) ? port : (Number.isInteger(state.port) ? state.port : null);
