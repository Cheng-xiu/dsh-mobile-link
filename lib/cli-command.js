...
import { extractHostname, probePort, waitPort } from './util.js';
...
async function cmdStart(options) {
  ...
  console.log('[warn] 若该实例不是由 dsh-mobile-link 启动的，手机访问 /api 可能被信任栅栏拦截 (403)。');
  console.log('[warn] 建议关闭现有实例后重新运行本命令（会自动带 --trusted-host 启动）。');
  const tunnel = await ensureTunnel(port);
  const result = await push(config, { url: tunnel.url, test: options.test, dryRun: options.dryRun });
  console.log(result);
  console.log('');
