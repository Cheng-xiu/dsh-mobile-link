...
async function cmdTunnel(options) {
  ...
  const port = toInt(options.port, config.webPort || 3080);
  if (options.stop) {
    // Only pass a port when the user explicitly gave one; otherwise let
    // stopTunnel fall back to the port recorded in the tunnel state.
    const stopped = stopTunnel(options.port === undefined ? null : port);
